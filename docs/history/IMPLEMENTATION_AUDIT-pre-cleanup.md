...
完整协议、配置字段、源码和来源见 [AppWorld说明](benchmarks/APPWORLD.md)，逐项排期见 [BENCHMARK_ROADMAP.md](BENCHMARK_ROADMAP.md)。这是上一轮AppWorld验收记录；本轮新增四项见文末扩展审计，P0算法没有改动。之前的审计另存 [history/IMPLEMENTATION_AUDIT-before-appworld.md](history/IMPLEMENTATION_AUDIT-before-appworld.md)。
...
- AppWorld阶段71项测试通过，无skip（本轮最新100项见文末）：原20、迁移12、batch对齐7、归因9、模型接受10、AppWorld13。P0历史记录原样保留；本轮日志与报告见 [appworld-tests.txt](validation/appworld-tests.txt)、[appworld-report.json](validation/appworld-report.json)。
...
## P1-1追加：四项benchmark适配审计

| Stage | Requirement | Status | Code location | Evidence | Gap / Action |
| ----- | ----------- | ------ | ------------- | -------- | ------------ |
| Benchmark | TB2官方容器/执行/verifier生命周期 | IMPLEMENTED / MOCK VERIFIED | [HarborSession](/data/miyapeng/harness-internalization/src/internalization/benchmarks/terminalbench.py:18)、[InternalizationAgent](/data/miyapeng/harness-internalization/src/internalization/benchmarks/harbor_agent.py:37) | 自有BaseAgent队列执行同一student动作；final后读取官方Trial reward；失败不伪装0分 | 实际Harbor 0.23.0/Docker/task未运行 |
| Benchmark | SWE-bench Pro工作区/补丁/官方评分 | IMPLEMENTED / MOCK VERIFIED | [DockerWorkspace](/data/miyapeng/harness-internalization/src/internalization/benchmarks/swebench.py:19)、[OfficialProScorer](/data/miyapeng/harness-internalization/src/internalization/benchmarks/swebench.py:47) | 镜像digest与grader tag身份检查；真实working tree diff；官方CLI与test artifacts；公开字段allowlist | 实际镜像、官方run_scripts与grader未运行 |
| Benchmark | HotpotQA固定检索和联合指标 | IMPLEMENTED / CPU FIXTURE VERIFIED | [metrics](/data/miyapeng/harness-internalization/src/internalization/benchmarks/hotpotqa.py:31)、[HotpotQAEnvironment](/data/miyapeng/harness-internalization/src/internalization/benchmarks/hotpotqa.py:45) | 实际自有worker进程执行合成search/lookup/final；数学例子验证joint F1 | 当前为distractor交互包装，fullwiki MISSING；官方数据和评分parity未运行 |
| Benchmark | LawBench 20题型及官方聚合 | IMPLEMENTED / MOCK VERIFIED | [OfficialLawBenchScorer](/data/miyapeng/harness-internalization/src/internalization/benchmarks/lawbench.py:23)、[native final evaluate](/data/miyapeng/harness-internalization/src/internalization/benchmarks/lawbench_final.py:16) | 不换成通用EM；完整题型重新调用官方评分；native final保留被官方跳过的样本，不编造单例reward | 官方依赖/ChERRANT未运行；无定义的单例不能直接进入现有四格统计 |
| 1 / 2 / 6 | 实际任务ID、hash、独立训练源、test隔离 | IMPLEMENTED | [import_tasks](/data/miyapeng/harness-internalization/src/internalization/benchmarks/importers.py:12)、[Catalog](/data/miyapeng/harness-internalization/src/internalization/benchmarks/common.py:81) | 默认test-only；显式独立train source生成>=30任务的三组retirement；test默认不能进入entrypoint选择 | 实际数据manifest未生成；外部训练源语义近重复仍需核验 |
| 3 / 4 / 5 | 新环境复用H− rollout与same-batch H+重评分和actor update | IMPLEMENTED / CPU MOCK VERIFIED | [entrypoint](/data/miyapeng/harness-internalization/src/internalization/training/entrypoint.py:57)、[新增测试](/data/miyapeng/harness-internalization/tests/test_benchmark_adapters.py:1) | 两批HotpotQA worker/CPU toy policy参数更新、snapshot/mask守卫；P0代码未修改 | 真实HF、external veRL和GPU NOT YET VERIFIED |
| Final | held-out评价不流回outer loop | IMPLEMENTED | [evaluate_benchmark.py](/data/miyapeng/harness-internalization/scripts/evaluate_benchmark.py:16)、[evaluate_lawbench.py](/data/miyapeng/harness-internalization/scripts/evaluate_lawbench.py:16) | 单独入口和显式final flag；LawBench完整最终评价不伪造逐样本reward | 官方test尚未运行 |

本轮新增29项测试，全量100项通过、无skip；原20项测试文件SHA256不变。三周期demo与AppWorld完成时90文件逐字节一致。记录见[benchmark-adapters-report.json](validation/benchmark-adapters-report.json)、[测试日志](validation/benchmark-adapters-tests.txt)，协议与命令见[ADAPTERS.md](benchmarks/ADAPTERS.md)。修改前审计保存在[history/IMPLEMENTATION_AUDIT-before-benchmark-adapters.md](history/IMPLEMENTATION_AUDIT-before-benchmark-adapters.md)。

**新增benchmark不会改变P0算法，但尚不能直接声称GPU实验就绪。** 真实引擎、官方数据、镜像及训练依赖仍待验收；HotpotQA的distractor包装和LawBench的原生单响应必须单独标注实验设定。
